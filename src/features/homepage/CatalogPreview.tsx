 ...
 * 
 * Homepage section showcasing featured products:
 * - Database-driven product display
 * - Responsive grid layout
 * - Add to cart functionality
 * - Loading and error states
 * - Performance optimized with memoization
 * - SEO-friendly product presentation
 */
...
const CatalogPreview: React.FC = React.memo(() => {
  ...
  /**
   * Handle navigation to full catalog
   */
  const handleViewAllDesigns = useCallback(() => {
    window.location.href = '/catalog';
  }, []);

  /**
   * Memoized product grid for performance
   */
    ...
    return (
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        {products.map((product) => (
          <div key={product.id} className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow card-hover">
