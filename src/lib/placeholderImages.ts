export const PLACEHOLDER_IMAGES = {
  stock_design: 'data:image/svg+xml;base64,',
  order: 'data:image/svg+xml;base64,',
  thumbnail: 'data:image/svg+xml;base64,',
};

export function getPlaceholderImage(type: 'stock_design' | 'order' | 'thumbnail' = 'stock_design'): string {
  return PLACEHOLDER_IMAGES[type];
}

export function getImageSrc(imageUrl: string | null | undefined, type: 'stock_design' | 'order' | 'thumbnail' = 'stock_design'): string {
  if (!imageUrl || imageUrl.trim() === '') {
    return getPlaceholderImage(type);
  }
  return imageUrl;
}
