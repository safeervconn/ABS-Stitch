export const PLACEHOLDER_IMAGES = {
  stockdesign: 'data:image/svg+xml;base64,',
  order: 'data:image/svg+xml;base64,',
  thumbnail: 'data:image/svg+xml;base64,',
};

export function getPlaceholderImage(type: 'stockdesign' | 'order' | 'thumbnail' = 'stockdesign'): string {
  return PLACEHOLDER_IMAGES[type];
}

export function getImageSrc(imageUrl: string | null | undefined, type: 'stockdesign' | 'order' | 'thumbnail' = 'stockdesign'): string {
  if (!imageUrl || imageUrl.trim() === '') {
    return getPlaceholderImage(type);
  }
  return imageUrl;
}
