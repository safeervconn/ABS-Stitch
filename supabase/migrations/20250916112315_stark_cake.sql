/*
  ...
  1. New Tables
    ...
    - `customers` - Customer-specific data and billing information
    - `sales_reps` - Sales representative data and performance metrics
    - `designers` - Designer profiles and specialties
...
BEGIN
    ...
    ELSIF COALESCE(NEW.raw_user_meta_data->>'role', 'customer') = 'sales_rep' THEN
        INSERT INTO sales_reps (id, employee_id) VALUES (NEW.id, 'SR' || EXTRACT(EPOCH FROM NOW())::text);
    ELSIF COALESCE(NEW.raw_user_meta_data->>'role', 'customer') = 'designer' THEN
        INSERT INTO designers (id, employee_id) VALUES (NEW.id, 'DS' || EXTRACT(EPOCH FROM NOW())::text);
    END IF;
